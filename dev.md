...
## ロールバック可能かつ、ユーザーの中断を防ぎ、新しいバージョンを展開するのに最適なオプション
...
ALB + ECS
## 統合タイプ
Lambdaプロキシ統合(AWS_PROXY): APIメソッドをLambda関数呼び出しアクションと統合できる。Lambda関数の呼び出しに適している
Lambdaカスタム統合およびその他全てのAWS統合(AWS): 統合リクエストとレスポンスの両方を設定し、データマッピングも設定する必要がある
HTTPプロキシ統合(HTTP_PROXY): バックエンドのHTTPエンドポイントにアクセスでき、クライアントとバックエンドがAPIGateway介入なしで直接対話できる
HTTPカスタム統合(HTTP): APIがバックエンドのHTTPエンドポイントを公開することを可能にする
モック統合(MOCK): リクエストをバックエンドに送信することなくレスポンスを返す、
## リリースのデプロイをテストする方法
...
-Count: APIリクエストの合計数
-IntegtationLatency: バックエンドにリクエスト中継してからバックエンドからレスポンスを受け取るまでの時間
-Latency: クライアントからリクエスト受け取ってからクライアントにレスポンス返すまでの時間
...
## デプロイプロセスを自動化する
sam deploy
以前は事前にsam packageコマンドを実行する必要があったが、deployコマンドのみで良くなった(試験問題ではこの答えを求められるかも)
...
## ユースケース
セッションデータを保持するのに適している
DAXを利用することでDyanamoDBmoインメモリ型DBとして活躍可能(でなければ、ElasticCache Redisが適する)
## アトミックカウンター
他の書き込み要求を妨げることなく、無条件にインクリメントされる数値属性
使用することで、UpdateItemを呼び出すたびに数値が増加する
注意: UpdateItemオペレーションは失敗したとき再試行するため、二重計上の可能性があるため厳密なカウンターが必要な場合は使用できない
## 単一パーティションに対して複数のインデックスを利用したクエリを実行したい
テーブル作成前に設定して同時に作成される必要がある
テーブルが作成される前にローカルセカンダリインデックスを追加する
## 現在テーブルのパーティションキーを設定しているが、全パーティションだけで全てのクエリを実行できるインデックスが必要
グローバルセカンダリインデックス
非キー属性のクエリを高速化できる
## CLIオプションでDynamoDBスキャンを実行し、特定の属性サブネットを取得操作したい
aws dynamodb get-item {省略} -- projection-expression "{属性名}"
複数属性の場合はカンマで区切る
...
## SQS拡張クライアントライブラリでできること
メッセージをS3に保存するか、メッセージサイズが256KBを超える場合のみ保存するかを指定
S3に保存されている単一メッセージオブジェクトを参照するメッセージを送信
該当するメッセージオブジェクトを取得・削除する
## lambda関数の失敗でDead Letter Queue(DLQ)を使用してSQSキューに送信する前に2回再試行する設定の場合、何回失敗したら移されるか
非同期呼び出し処理が3回失敗した場合
## デフォルトのメッセージ保存期間
4日(保存期間は最大14日)
...
## S3にファイルをアップロードして、保存中のデータを保護する方法
アップロードリクエストに x-amz-server-side-envryptionヘッダーにaws:kmsの値を含むリクエストを送信する必要がある
## ヘッダー
